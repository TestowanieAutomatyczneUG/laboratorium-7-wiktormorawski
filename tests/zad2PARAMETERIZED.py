...
def test_Song_Lyrics_return_all(expected):
    tmp = Song_lyrics()
    assert_equal(tmp.All(), expected)

class Song_LyricsPARAMETRIZED(unittest.TestCase):
    def setUp(self):
        self.tmp = Song_lyrics()
    ...
    def test_byLine(self, input, expected):
        self.assertEqual(self.tmp.byLine(input), expected)
class Song_LyricsTEST_ALL(unittest.TestCase):
    def setUp(self):
        self.tmp = Song_lyrics()
    @parameterized.expand([
        ('On the first day of Christmas my true love gave to me: a Partridge in a Pear Tree.' +
    ...
    def test_Song_Lyrics_return_all(self, expected):
        assert_equal(tmp.All(), expected)

tmp = Song_lyrics()
@parameterized_class(('input1', 'input2', 'expected'),[
    (1, -3, ValueError),
    ("1", 2, ValueError),
    (1, "2", ValueError),
    (-1, 11, ValueError),
...
class Song_lyrics_ByLineInterval(unittest.TestCase):
    def setUp(self):
        self.tmp = Song_lyrics()
    def test_exceptions(self):
        assert_raises(self.expected, self.tmp.byLineInterval, self.input1, self.input2)
    def tearDown(self):
        self.tmp = None
@parameterized_class(('input1', 'input2', 'expected'),[
